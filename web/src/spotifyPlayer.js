...
/** Detect Chrome blocking Widevine before the SDK hangs or throws initialization_error. */
export async function probeSpotifyDrmSupport() {
  if (typeof window === "undefined") {
  ...
  try {
    ...
    return {
      ok: false,
      reason: e?.name || "widevine-blocked",
      message: e?.message || "",
    };
  }
}

export function formatSpotifyDrmProbeError(probe) {
  if (!probe || probe.ok) return "";
  if (probe.reason === "insecure-context") {
    return "Spotify playback requires HTTPS — open hit4hit.app with https://";
  }
  if (probe.reason === "no-eme") {
    return "This browser can't play Spotify DRM — try Chrome or Edge on desktop";
  }
  return "Chrome blocked Spotify DRM — open chrome://settings/content/protectedContent and allow protected content, then refresh";
}
...
export async function createSpotifyPlayer(getAccessToken) {
  ...
  const drm = await probeSpotifyDrmSupport();
  if (!drm.ok) {
    throw new Error(formatSpotifyDrmProbeError(drm) || "Spotify DRM unavailable");
  }
