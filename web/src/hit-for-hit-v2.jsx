...
import {
  ...
  SPOTIFY_PREVIEW_ONLY,
} from "@shared/constants/musicConstants.js";
import { FINAL_PUNISHMENTS, ROUND_PUNISHMENTS } from "@shared/constants/punishments.js";
...
export default function HitForHit() {
  ...
  const musicServiceHint = usesAppleMusic
    ...
    : SPOTIFY_PREVIEW_ONLY
      ? "Everyone can search artists and songs. Host hears 30s previews — pick each song from search."
  ...
  const startHostPlayback = useCallback(() => {
    ...
    const spotifyPreviewOnly = !usesAppleMusic && SPOTIFY_PREVIEW_ONLY;
    ...
    const usePreviewPath =
      hasPreview &&
      (spotifyPreviewOnly || !appleWantsFull || mobileSafariPreview);
    ...
    const primedPreview = usePreviewPath
        ? playPreviewFromUserGesture(resolvedNow.preview)
        : null;

    const preferGesturePreview = Boolean(primedPreview) && usePreviewPath;
      ...
      try {
        ...
        if (spotifyConnectPromise) {
          try {
            ...
            const hasPreview = Boolean(resolved?.preview);
            if (hasPreview) {
              showToast(
                "Full Spotify playback blocked — playing 30s preview instead",
                5000
              );
              spotifyReady = false;
            } else {
        ...
        const preferFullTrack =
          appleWantsFull && !mobileSafariPreview
            ? true
            : !spotifyPreviewOnly && spotifyReady;
        ...
        if (result?.type === "none") {
          ...
          } else if (!spotifyPreviewOnly && !spotify.loggedIn) {
          ...
          } else if (spotifyPreviewOnly && !resolved?.preview) {
            showToast("Pick this song from search results — typed titles often have no preview");
          } else {
        ...
        if (result.type === "preview") {
          showToast(
            mobileSafariPreview || spotifyPreviewOnly
              ? "Playing 30s preview"
              : appleWantsFull
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            {usesAppleMusic ? (
              ...
              <span className="bf" style={{fontSize:10,color:MUTED3}} title={SPOTIFY_PREVIEW_ONLY ? "Host plays 30s previews from search" : "Host logs in to Spotify for playback"}>{musicLabel}</span>
            )}
          </div>
          <div
            style={{
      ...
      <main style={{maxWidth:500,margin:"0 auto",padding:"1.25rem 1rem 3rem"}}>
        ...
        {screen==="lobby" && gs && (
          <div className="slide-up">
            ...
            {isHost && !usesAppleMusic && SPOTIFY_PREVIEW_ONLY && (
              <div className="card" style={{padding:"0.85rem 1rem",marginBottom:10,borderColor:"#5b21b6"}}>
                <div className="bf" style={{color:"#d8b4fe",fontSize:12,lineHeight:1.5}}>
                  Pick each song from search — 30 second previews play on your device. No Spotify login needed.
                </div>
              </div>
            )}

            {isHost && !usesAppleMusic && !SPOTIFY_PREVIEW_ONLY && !spotify.loggedIn && (
              <div className="card" style={{padding:"0.85rem 1rem",marginBottom:10,borderColor:"#5b21b6"}}>
                <div className="bf" style={{color:"#d8b4fe",fontSize:12,lineHeight:1.5}}>
            ...
            {isHost && !usesAppleMusic && !SPOTIFY_PREVIEW_ONLY && spotify.loggedIn && !spotify.playbackReady && (
              <div className="card" style={{padding:"0.85rem 1rem",marginBottom:10,borderColor:"#5b21b6"}}>
                <div className="bf" style={{color:"#d8b4fe",fontSize:12,lineHeight:1.5}}>
                  ...
                  {spotify.drmBlocked && (
                    <>
                      {" "}
                      You can still host rounds using 30s previews — pick each song from search results.
                    </>
                  )}
                </div>
              </div>
