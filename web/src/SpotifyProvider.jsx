...
import {
  createSpotifyPlayer,
  formatSpotifyDrmProbeError,
  isDesktopSpotifyClient,
  isSpotifyAppFallbackClient,
  loadSpotifySdk,
  probeSpotifyDrmSupport,
  resetSpotifySdkLoad,
  nudgeSpotifyApp,
...
export function SpotifyProvider({ children }) {
  ...
  useEffect(() => {
    ...
    probeSpotifyDrmSupport()
      .then((probe) => {
        if (cancelled) return;
        const blocked = !probe.ok;
        setDrmBlocked(blocked);
        if (blocked) {
          setPlayerStatus((prev) =>
            prev && !/Tap Connect|Finishing Spotify login/i.test(prev)
              ? prev
              : `${formatSpotifyDrmProbeError(probe)} — 30s previews still work if you pick songs from search`
          );
        }
      })
      .catch(() => {
        if (!cancelled) setDrmBlocked(false);
      });
